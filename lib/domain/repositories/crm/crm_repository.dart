...
class CrmRepository implements ICrmRepository {
  ...
  Future<List<dynamic>> getCrmList({String? busCRMTypeid, String? busCRMSalesStatusid}) async {
    try {
      // final auth = "jvZVSzvERAlIB9tiyK5c/1Bc3z3PzMWsnuh7O16yzQdA/x/HXBFwPqhd4fc6FSaK/y8KKArnmwFX2oWNa/IukeIqY/Uh3HS+ZhkHltuvgIU="; // final auth = LocalStorage.getToken();
      final response = await _apiService.post(ApiConstants.crmGetList, {'Auth': _apiService.token, 'BusCRMTypeid': busCRMTypeid ?? '', 'BusCRMSalesStatusid': busCRMSalesStatusid ?? ''});
  ...
  Future<Map<String, dynamic>> getCrmById(String busSaleCRMID) async {
    try {
      // final auth = "jvZVSzvERAlIB9tiyK5c/1Bc3z3PzMWsnuh7O16yzQdA/x/HXBFwPqhd4fc6FSaK/y8KKArnmwFX2oWNa/IukeIqY/Uh3HS+ZhkHltuvgIU="; // final auth = LocalStorage.getToken();
      final response = await _apiService.post(ApiConstants.crmViewById, {'Auth': _apiService.token, 'BusSaleCRMID': busSaleCRMID});
      return response;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> addCrm({
    ...
    try {
      // final auth = "jvZVSzvERAlIB9tiyK5c/1Bc3z3PzMWsnuh7O16yzQdA/x/HXBFwPqhd4fc6FSaK/y8KKArnmwFX2oWNa/IukeIqY/Uh3HS+ZhkHltuvgIU="; // final auth = LocalStorage.getToken();
      final data = {'Auth': _apiService.token, 'BusCRMTypeid': busCRMTypeid, 'FollowDate': followDate, 'CRMParty': crmParty, 'MobileNumber': mobileNumber, 'CRMNotes': crmNotes};
